...
router.get(paths.getById, async (req: Request, res: Response) => {
  ...
  if (!transaction) {
    throw new DataNotFound(Transaction.name);
  }
  res.status(StatusCodes.OK).json(transaction);
});

router.post(paths.send, async (req: Request, res: Response) => {
...
router.post(paths.mine, async (req: Request, res: Response) => {
  const address = req.body.address;
  const amount = req.body.amount;
